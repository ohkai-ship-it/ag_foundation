...
## Note 8 — Post-implementation phase has no canonical timeline

**Observed by:** Kai (PM, ag_foundation)
**Date:** 2026-04-06
**Context:** Sprint 18 close. After all 3 AFs were implemented and committed, the post-implementation phase (status finalization, review creation, INDEX updates, PR creation) was disorganized. Jacob performed steps in a different order than expected at each stage, requiring repeated corrections.

**Problem:** The governance docs describe *what* must happen at sprint close (SPRINT_MANUAL §7–§8) but do not define a canonical **timeline** — a strict ordered sequence of tasks covering the full work cycle from sprint start through sprint close. Each section (§7 Pre-Merge Finalization, §8 Sprint Close Ritual) has checklists, but:

1. **No single ordered sequence** ties the phases together. The agent has to mentally merge §7 + §8 + §6 and decide on ordering.
2. **No explicit handoff points** between phases (e.g., "all AF statuses updated → now create review → now create PR → now update sprint description status").
3. **The agent improvises the order** each time, leading to inconsistent execution and PM frustration.

This is the same root cause as Note 4 (inter-sprint phase is unstructured) but applies to the *entire* work cycle, not just planning.

**Impact:** High. Every sprint close becomes a multi-prompt correction loop. The PM has to shepherd the agent through a sequence that should be deterministic. This defeats the purpose of documented workflows.

**Suggested fix directions:**
1. Create a **single canonical timeline** (numbered step list) covering the full sprint lifecycle: start ritual → AF implementation loop → post-implementation finalization → review → PR → merge. Each step references the detailed section (e.g., "Step 12: Create S##_REVIEW.md (see §8.6)").
2. Consider making this a machine-readable checklist that agents can track (e.g., a JSON schema or markdown checklist template that gets filled per sprint).
3. Separate "what to do" (existing sections) from "when to do it" (the new timeline) — both are needed.

---

## Note 9 — AF metadata timestamps still lack time component (recurring)

**Observed by:** Kai (PM, ag_foundation)
**Date:** 2026-04-06
**Context:** Sprint 18 close. AF-0144, AF-0145, AF-0147 metadata fields (`Started:`, `Completed:`) were filled with date-only values (e.g., `2026-04-06`) instead of datetime values.

**Problem:** This is a recurrence of Note 3. Despite the observation being logged, no corrective action was taken in the templates or governance docs. The agent continues to write date-only values because:
1. The template fields don't show an explicit format example (just `Started:` with blank)
2. No validation rule enforces the format
3. The agent defaults to the simplest date format unless explicitly instructed otherwise

**Impact:** Low per instance, but the pattern of "feedback logged → not actioned → recurs" is itself a governance gap. Feedback notes need a triage/action pipeline.

**Suggested fix directions:**
1. Update the backlog item template: change `# Started:` to `# Started: YYYY-MM-DDTHH:MM` (format as placeholder)
2. Same for `# Completed:`, `# Created:`
3. Consider a "feedback triage" ritual: at each GVS sprint start, review open feedback notes and convert actionable ones to AFs
