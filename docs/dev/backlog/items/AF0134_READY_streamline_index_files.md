...
## Acceptance Criteria
- [ ] INDEX_BACKLOG.md has no `Filename` column
- [ ] INDEX_BACKLOG.md has no Active/Done section split — single table per sprint, Status column is the filter
- [ ] INDEX_BUGS.md has no `Filename` column (or equivalent redundancy)
- [ ] All links in INDEX files resolve to existing files (both old- and new-convention names)
...
- [ ] SPRINT_MANUAL INDEX update ritual updated to reflect new 2-column approach
