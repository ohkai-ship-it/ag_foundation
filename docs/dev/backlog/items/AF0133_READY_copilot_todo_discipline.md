...
## Acceptance Criteria
- [ ] `.github/copilot-instructions.md` exists (create if absent, update if present)
- [ ] File contains: ToDo list discipline rule with title format `AF-####: <AF Title>`
- [ ] File contains: mark in-progress before starting, completed immediately after finishing
- [ ] File contains: one active Todo at a time constraint
