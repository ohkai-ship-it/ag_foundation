...
Implement governance simplification across Sprint 16 (8 AFs) to achieve ~10 min/sprint ceremony target:

1. **Eliminate filename-status coupling (AF-0129):** New files use immutable filenames without status tokens. Status tracked in exactly 2 places: internal metadata + INDEX row. Legacy files keep their names.

2. **Drop redundant sprint artifacts (AF-0130):** Replace 4-file sprint structure with MECE pair: `S##_DESCRIPTION.md` (planning) + `S##_REVIEW.md` (outcomes). GitHub PR is the canonical PR artifact.

3. **Enhance templates (AF-0131):** Add time/model logging, docs impact checklist, inline decision record to all templates.

4. **Codify HITL framework (AF-0132):** 15 named gates (G1–G15) in FOUNDATION_MANUAL §10. Constitutional principle: "Jacob may always escalate; Kai may always override."

5. **Streamline INDEX files (AF-0134):** Add Link column as sole file reference. No row moves on status change — update in place. Historical entries exempt.

6. **Historical record immutability (AF-0138):** Pre-v1.3 entries are immutable. No renames, restructuring, or normalization of historical records.

7. **Unify Governance System Version (AF-0136):** All governance docs bumped to GSV v1.3. Single version number answers "what governance system is running?"

8. **Governance automation deferred (AF-0135):** `gov.py` validation script deferred to backlog. Consolidate first, automate after the system proves usable.
