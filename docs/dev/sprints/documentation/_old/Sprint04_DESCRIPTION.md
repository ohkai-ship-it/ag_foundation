...
- **Tech lead:** Jeff
- **Implementer:** Jacob
- **Primary goal:** Migrate from `/docs/dev/` to `/docs/dev/` and enforce a deterministic sprint + artifact workflow (folder structure, naming conventions, templates, CI/ruff enforcement).
...
- clean separation between legacy `/docs/dev/` and the new canonical `/docs/dev/`.

**Important migration rule:** During this sprint, `/docs/dev/` remains the “legacy baseline”. `/docs/dev/` becomes the “new baseline”. We do not delete `/docs/dev/` until the sprint review accepts the migration.
...
### Must-have (P0)
- **AF0039** — Create new `/docs/dev` structure + move ARCHITECTURE/CLI_REFERENCE to repo root + rename indexes
...
- [ ] New canonical structure exists under `/docs/dev/` and is internally consistent
...
- **Canonical:** `/docs/dev/` (all new work lands here)
...
- PR1 (AF0039): Create `/docs/dev` skeleton + root doc moves + index renames
...
- Sprint folder (canonical):
  ...
  - `docs/dev/sprints/documentation/Sprint04_process_hardening/artifacts/`

- AF items (canonical):
  - `docs/dev/backlog/items/AF0039_Ready_...md` etc.
