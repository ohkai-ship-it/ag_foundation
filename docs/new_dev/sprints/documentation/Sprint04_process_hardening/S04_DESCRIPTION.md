...
> **Folder:** `/docs/new_dev/sprints/documentation/Sprint04_process_hardening/`
...
## 2) Sprint goal
Migrate from `/docs/dev/` to `/docs/new_dev/` and enforce a deterministic sprint + artifact workflow (folder structure, naming conventions, templates, CI/ruff enforcement).
...
### Must-have (P0)
- AF0039 — Create new `/docs/new_dev` structure + move ARCHITECTURE/CLI_REFERENCE to repo root + rename indexes (Done: prep)
...
- [x] Check AFs in `/docs/new_dev/backlog/items/`
...
- PR1: AF0039 — Create `/docs/new_dev` skeleton + root doc moves + index renames (Done: prep)
