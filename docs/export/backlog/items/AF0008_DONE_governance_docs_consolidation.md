...
- **Phase:** 9 (depends on AF-0001 through AF-0006, AF-0010)
...
- All **authoritative** governance docs internally consistent with new rules **(historical entries exempt — see AF-0010)**
...
- Changes to `src/`, `tests/`, or `scripts/gov.py` (AF-0007, deferred)
- Renaming, restructuring, or normalizing any pre-v1.3 entries (INDEX rows, AF/BUG files, sprint docs) — see AF-0010 transition brief
...
- [x] SPRINT_MANUAL references HITL Framework (AF-0004) at all relevant decision points
- [x] `FOLDER_STRUCTURE_0.3.md` exists; documents current folder layout including both filename conventions
- [x] ADR-0001 exists in `docs/dev/decisions/files/` with ACCEPTED status
- [x] INDEX_DECISIONS updated with ADR-0001 row
...
All Sprint 01 AF files (AF0001–AF0008) were created with v0.2 and may be bumped to v1.3 as part of this AF.
...
- `docs/dev/decisions/files/ADR001_governance_simplification.md` (new file)
...
- Sprint 01 AF files AF0001–AF0010 (bump to v1.3)
...
- **Decision:** Governance System Version (GSV) v1.3 assigned to all governance docs after S01.
...
- **Follow-ups:** AF-0007 (gov.py) returned to uncategorized backlog
...
- `docs/dev/decisions/files/ADR001_governance_simplification.md` — **new file** (governance simplification ADR)
- `docs/dev/decisions/INDEX_DECISIONS.md` — ADR-0001 row + v1.3 bump
...
- `docs/dev/sprints/documentation/Sprint01_governance_simplification/S01_DESCRIPTION.md` — branch name fix, state update
