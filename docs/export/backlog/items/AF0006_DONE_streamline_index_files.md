...
- **Phase:** 3 (depends on AF-0001 — naming convention must be settled first)
...
- All links resolve to correct files for both old-convention and new-convention filenames (per AF-0001)
...
## Acceptance Criteria
- [x] INDEX_BACKLOG.md has no `Filename` column (Sprint 01+ and unprioritized backlog; historical sprints preserved)
- [x] INDEX_BACKLOG.md has no Active/Done section split (Sprint 01+ uses Status column as filter; historical layout preserved)
...
### Coordination with AF-0001

AF-0001 defines the new filename convention. AF-0006 must use both conventions in link column (legacy files keep old names). Check that links using new convention (no status token) also resolve correctly.
...
**Scope adjustment:** Per governance rule, structural changes (Filename→Link column, Active/Done merge) apply only to Sprint 01+ tables and the unprioritized backlog. All historical sprint entries (≤15) are preserved exactly as-is to maintain evolution visibility.

### Changes made
1. **INDEX_BACKLOG.md** — Header metadata updated (Foundation Rule text, Naming reference, Linking convention). Sprint 01/17 and unprioritized backlog tables use `Link` column. Historical sprints untouched.
...
4. **INDEX_SPRINTS.md** — Header metadata updated. Structure section fixed: removed stale `S##_PR_01.md` and `S##_REVIEW_01.md` references, replaced with current `S##_REVIEW.md` convention per AF-0002.
