# Sprint 01 Observations — Governance System Learnings
# Version number: v0.1
# Created: 2026-04-05
# Author: Kai (PM), captured by Jeff (Tech Lead)
# Context: Observations collected during Sprint 01 (governance_simplification) implementation
...
**Resolution:** AF-0010 (v1.3 transition brief) created mid-sprint. Tiered placement: canonical rule in FOUNDATION_MANUAL, one-sentence pointer in SPRINT_MANUAL §2, HTML comment in each INDEX file header. ~4 lines of new content total.

### 1.2 Commit discipline failure
Jacob did not commit a single AF through the first 7 completed items. All changes sat in the working tree with no git safety net. The "1 commit per AF" rule from S01_DESCRIPTION §6 was entirely unenforced.
...
### 1.3 AF-0008 ambiguity
The goal statement "All governance docs internally consistent with new rules" was ambiguous enough to imply normalizing historical entries. AF-0008 was rewritten to say "All **authoritative** governance docs internally consistent with new rules **(historical entries exempt — see AF-0010)**" with an explicit Non-goals line about pre-v1.3 entries.
...
Jacob treats G4 (AF completion approval) as "I cannot initiate a commit until the user tells me to" — a passive blocker. The correct behavior is proactive: "I've completed AF-0001. Here's what changed: [summary]. All AC met. Approve for commit?"
...
### 2.4 AF-0007 (gov.py) deferred
Decision: consolidate first (AF-0008), automate later. Rationale:
...
- AF-0007 moved to unprioritized backlog; may be dropped if tests cover it
...
5. **Files in `docs/dev/additional/`** with old version numbers weren't updated in S01 — no AF scoped them

These conventions need to be codified in FOUNDATION_MANUAL or FOLDER_STRUCTURE_0.3. Candidate for AF-0008 scope or a follow-up AF.
...
- Sprint 01 description in PROJECT_PLAN still says "Skill Catalog Expansion" — factually stale
...
- Keep this in mind for the deployability phase — not Sprint 01 scope
...
4. **Sprint conventions change mid-sprint.** Just like Sprint 01's D11 — the sprint delivers new conventions that could retroactively apply to itself.
...
This also solves the "D11 problem" from Sprint 01: you're always building the next version while running under the current one. In `framework-dev` mode, that's the standard operating pattern, not an exception.
...
**Timing decision:** Kai decided to outsource the governance system immediately after Sprint 01 close — not wait for a validation sprint. Rationale: the governance system will iterate faster without ag_foundation's runtime overhead, and the feedback loop design problem exists regardless of timing.
...
1. **Governance Bug Reports** — consumer discovers a rule that's broken, ambiguous, or missing (e.g., S01's historical edits problem). Filed as a bug *in the governance repo*, tagged with the consumer project that surfaced it.
...
Sprint 01 had three scope changes mid-flight: AF-0010 added, AF-0007 deferred, AF-0008 rewritten. All three were necessary and well-reasoned — but the process for making them was entirely ad hoc. There is no governed procedure for changing sprint scope after the sprint has started.

### What happened
1. **AF-0010 created** — new P1 AF added mid-sprint to fill a gap (v1.3 transition brief)
2. **AF-0007 deferred** — P2 AF moved to unprioritized backlog (consolidate before automating)
3. **AF-0008 rewritten** — Goal, Non-goals, phase dependencies, and file references updated to reflect AF-0010 and AF-0007 changes

Each change required updates across multiple files: the AF itself, INDEX_BACKLOG, S01_DESCRIPTION (scope, execution diagram, commit plan, key references), and in AF-0008's case the review file as well. This was done correctly but only because Jeff was tracking it manually.

### What's missing
- **No formal scope change protocol.** SPRINT_MANUAL has no section for mid-sprint AF additions, deferrals, or rewrites.
- **No traceability.** The S01_DESCRIPTION was edited in place — there's no record of *when* or *why* the scope changed, only the final state. Someone reading the description after sprint close would never know AF-0010 was a mid-sprint addition.
- **No impact checklist.** When scope changes, which files must be updated? Currently this is tribal knowledge:
  ...
  - S01_DESCRIPTION (scope §3, execution diagram §4, commit plan §6, references §8)
...
## 6) Summary of Sprint 01 Mid-Sprint Actions Taken

| Action | Detail | Status |
|---|---|---|
| AF-0010 created | v1.3 transition brief — tiered immutability rule | DONE — committed |
| AF-0008 rewritten | Historical immunity in Goal/Non-goals, AF-0007 refs removed, phase dependency updated | DONE |
| AF-0007 deferred | Moved from S01 scope to unprioritized backlog | DONE |
| INDEX_BACKLOG updated | AF-0010 added at order 7, AF-0007 moved to backlog, AF-0008 reordered to 8 | DONE |
| S01_DESCRIPTION updated | Scope §3, execution diagram §4, commit plan §6, key references §8 | DONE |
| Commits caught up | AF-0001 through AF-0010 committed in execution order | DONE |
...
These items need a decision during the S01 review — they are not yet resolved:

1. **Versioning conventions (§3):** Codify in AF-0008 or defer to follow-up AF?
2. **Phase-anchored gates (§2.1):** Should AF-0008 add phase gates to SPRINT_MANUAL, or is this a separate AF for Sprint 17?
3. **Test scheduling (§2.2):** When do we address this — Sprint 17 or a dedicated hardening sprint?
4. **PROJECT_PLAN staleness (§4.4):** Update as part of AF-0008's living reference sweep, or accept it as stale until the roadmap is formalized?
5. **Governance standalone project:** ~~At what point do we create the separate repo?~~ **RESOLVED** — Kai decided: immediately after Sprint 01 close. No waiting period.
6. **Scope change protocol (§5):** Codify in SPRINT_MANUAL as part of AF-0008, or create a follow-up AF?
...
9. **GVS project plan drafted (§4):** See `GVS_PROJECT_PLAN_0.1.md` — covers extraction, architecture, consumer model, bootstrap strategy, phased roadmap. Status: DRAFT, needs review during S01 Section B.
