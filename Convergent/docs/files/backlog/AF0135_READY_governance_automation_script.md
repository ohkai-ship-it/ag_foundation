...
- **Phase:** 7 (depends on AF-0129 + AF-0134)

---

## Problem

Even after AF-0129 (no filename renames) and AF-0134 (streamlined INDEX), creating a new AF or updating a status still requires 2 manual edits across 2 files. Creating an AF requires: (1) copy template, (2) fill metadata, (3) add INDEX row with correct link. These are mechanical, error-prone, and account for ~5–10 min/sprint in ceremony data.
...
## Acceptance Criteria
- [ ] `python scripts/gov.py check` passes on current repo state after S16 changes
...
- [ ] `gov.py check` produces no false positives on new-format filenames (AF-0129 convention)
