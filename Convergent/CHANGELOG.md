...
## [v1.3.2] — 2026-04-17 (in progress)

Structural overhaul: GS reorganized into Part I/II/III with §-numbering, SP and LR renumbered to Phase.Step.Task scheme, server immutability codified, handoff protocol extracted.

### Added
- GS §1.1: Server Immutability rule — foundation/ is read-only for consumers
- GS Part I/II/III index (table of contents) at top of GOVERNANCE_STANDARD.md
- SESSION_CHAIN.md: §3.4 Inter-Agent Handoff protocol extracted from GS §2.3
- SP Appendix C: Structural Quick Reference (renamed from duplicate Appendix A)

### Changed
- GS restructured into Part I (Intro: §1–§3), Part II (Phases: §4–§9, §10 Gates, §11 Status), Part III (§12 Consumer Rules) — full renumbering from old §0–§10 layout
- SP renumbered: all phases use `N.x Step:` / `N.x.y Task:` scheme (was `Step N:` / `Step N.N:`)
- LR renumbered: same Phase.Step.Task scheme as SP
- Cross-refs updated across 6 files: GS internal (9), SP (16), LR (1), FOLDER_STRUCTURE (2+bugfix), PROJECT_PLAN_TEMPLATE (2), PROJECT_CONTEXT (4)
- SP/LR inline refs updated to new step numbers (loop refs, test-fail refs, manifest check refs, living docs refs)
- Template cross-refs updated: SPRINT_REVIEW, PULL_REQUEST, SPRINT_DESCRIPTION templates
- SESSION_CHAIN cross-refs updated to new SP step numbers

### Fixed
- FOLDER_STRUCTURE: §8.5 reference was pointing to Testing Framework instead of Historical Record Immutability (pre-existing bug, now §1.4)
- SP: duplicate "Appendix A" naming collision resolved (second → Appendix C, subsections A.x → C.x)
